...
export default function AIChatModal({ isOpen, onClose, initialQuery = '' }: AIChatModalProps) {
	...
	const { messages, addMessage, clearMessages, hasMessages } = useChatContext();
	...
	useEffect(() => {
		if (isOpen) {
			...
			if (initialQuery) hasProcessedInitialQuery.current = false;
		}
	...
	const handleSendMessage = async (content: string = inputValue) => {
		...
		setTimeout(() => {
			addMessage(t('aiChat.devResponse', { query: content.trim() }), 'ai');
			setIsTyping(false);
		}, 1500);
	...
	return (
		<div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
			...
			<div className="relative w-full max-w-3xl h-[80vh] bg-white/5 backdrop-blur-2xl border border-white/20 rounded-3xl shadow-[0_8px_32px_rgba(0,0,0,0.4)] overflow-hidden flex flex-col">
				<div className="flex items-center justify-between px-4 md:px-6 py-4 border-b border-white/10">
					<div className="flex items-center gap-3">
						<div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#00d2ff]/20 to-[#0099cc]/20 backdrop-blur-xl border border-white/20 flex items-center justify-center">
							<img src="/ia.png" alt="Doge AI" className="w-6 h-6 object-cover" />
						</div>
						<div>
					...
					<div className="flex items-center gap-2">
						{hasMessages && (
							<button
								...
								className="w-9 h-9 rounded-lg bg-white/5 hover:bg-red-500/20 border border-white/10 hover:border-red-500/30 text-white/60 hover:text-red-400 transition-all flex items-center justify-center cursor-pointer"
						...
						<button
							onClick={onClose}
							className="w-9 h-9 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 text-white/60 hover:text-white transition-all flex items-center justify-center cursor-pointer"
						>
							<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
						...
						<div className="h-full flex flex-col items-center justify-center text-center space-y-6 px-4">
							<div className="relative">
								...
								<div className="relative w-16 h-16 bg-gradient-to-br from-[#00d2ff]/30 to-[#0099cc]/30 backdrop-blur-xl border border-white/20 rounded-2xl flex items-center justify-center">
									<img src="/ia.png" alt="Doge AI" className="w-10 h-10 object-cover" />
								</div>
							</div>
							<div className="space-y-2">
								<h3 className="text-xl font-semibold text-white">{t('aiChat.welcome')}</h3>
								<p className="text-sm text-white/60 max-w-md leading-relaxed">{t('aiChat.description')}</p>
							</div>
								...
								<div className="grid grid-cols-1 md:grid-cols-2 gap-2">
									{suggestedQuestions.map((question, i) => (
										<button
											...
											className={`group bg-white/5 hover:bg-white/10 backdrop-blur-sm border border-white/10 hover:border-[#00d2ff]/40 rounded-xl px-4 py-3 text-left text-sm text-white/70 hover:text-white transition-all cursor-pointer ${i >= 2 ? 'hidden md:block' : ''}`}
										>
									...
									<div className={`max-w-[75%] rounded-2xl px-3 py-2 ${
										...
										{msg.type === 'ai' && (
											<div className="flex items-center gap-2 mb-1.5 opacity-60">
												<img src="/ia.png" alt="Doge" className="w-3.5 h-3.5 object-cover" />
							...
							{isTyping && (
								<div className="flex justify-start animate-fadeIn">
									<div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl px-3 py-2">
										<div className="flex items-center gap-2">
											<img src="/ia.png" alt="Doge" className="w-3.5 h-3.5 object-cover opacity-60" />
											<div className="flex gap-1">
												{[0, 150, 300].map((delay) => (
													<span key={delay} className="w-1.5 h-1.5 bg-[#00d2ff]/60 rounded-full animate-bounce" style={{ animationDelay: `${delay}ms` }} />
												))}
											</div>
										</div>
									</div>
				...
				<div className="border-t border-white/10 px-4 md:px-6 py-4 bg-white/5 backdrop-blur-xl">
					<form onSubmit={(e) => { e.preventDefault(); handleSendMessage(); }} className="flex items-center gap-2">
						<input
							...
							maxLength={1000}
							className="flex-1 bg-white/5 backdrop-blur-sm border border-white/10 focus:border-[#00d2ff]/50 rounded-xl px-4 py-3 text-base md:text-sm text-white placeholder-white/40 outline-none transition-all"
							disabled={isTyping}
						/>
						<button
							type="submit"
							disabled={!inputValue.trim() || isTyping}
							className="bg-gradient-to-r from-[#00d2ff] to-[#0099cc] hover:from-[#0099cc] hover:to-[#00d2ff] disabled:from-white/10 disabled:to-white/10 disabled:cursor-not-allowed text-[#0b0d12] disabled:text-white/40 w-10 h-10 rounded-xl font-bold transition-all flex items-center justify-center shadow-lg shadow-[#00d2ff]/20 disabled:shadow-none cursor-pointer"
						>
							<svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
					...
					<p className="text-[10px] text-white/30 mt-2 text-center">{t('aiChat.hint')}</p>
				</div>
			</div>
		</div>
	);
