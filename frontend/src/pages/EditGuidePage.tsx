...
export default function EditGuidePage() {
	...
	const fetchGuide = async () => {
		try {
			const response = await fetch(`/api/guides/${slug}?userId=${user?.id}`)
			const result = await response.json()
			
			if (result.success && result.data.isOwner) {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (result.success) {
				setToast({ message: t('guides.editor.saveSuccess'), type: 'success' })
				const newSlug = result.data.slug
				setTimeout(() => navigate(`/guias/${newSlug}`), 1500)
			} else {
						...
						<div className="animate-pulse space-y-6">
							...
							<div className="bg-white/5 border border-white/10 rounded-2xl p-6 space-y-4">
								...
								<div className="grid grid-cols-3 gap-2">
									{[...Array(6)].map((_, i) => <div key={i} className="h-10 bg-white/10 rounded-lg" />)}
								</div>
							</div>
	...
	return (
		<>
			<SEO
				title={t('guides.editor.editTitle')}
				description={t('guides.editor.editDescription')}
			/>
				...
				<main className="flex-1 container mx-auto px-4 py-24">
					...
					<div className="max-w-5xl mx-auto mb-6">
						<div className="flex items-center gap-2 text-sm text-gray-400">
							...
							<span>›</span>
							<Link to={`/guias/${slug}`} className="hover:text-[#00d2ff] transition cursor-pointer truncate max-w-[150px]">
								{title}
							</Link>
							<span>›</span>
							<span className="text-white">{t('guides.editor.editTitle')}</span>
						</div>
					</div>
					...
					<div className="max-w-5xl mx-auto mb-8">
						<h1 className="text-4xl font-bold text-white mb-2">
							{t('guides.editor.editTitle')}
						</h1>
						<p className="text-gray-400">
							{t('guides.editor.editDescription')}
						</p>
					</div>
